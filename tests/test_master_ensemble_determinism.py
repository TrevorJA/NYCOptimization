"""tests/test_master_ensemble_determinism.py - Master-ensemble regeneration determinism gate.

The regenerate-on-demand design (methods §3.2/§7) requires that master realization ``k`` be
reproducible bit-for-bit from ``(master_seed, k)`` alone, invariant to how the realization range is
partitioned. This test generates a tiny CMIP6-forced master (``store_daily=True``), then regenerates
one realization in isolation and asserts it equals the staged HDF5 slice (post float32/HDF5
round-trip). Because the full master generates realizations in blocks while ``regenerate_realization``
generates a single index alone, equality proves partition-invariance through the Kirsch -> Nowak ->
...
from scengen.master_ensemble import MasterEnsembleConfig

slow = pytest.mark.slow

# Tiny master: 4 forcing profiles x 2 realizations = 8 realizations of 2-yr records.
_N_FORCING, _R, _L, _SEED = 4, 2, 2, 0
_REGEN_INDEX = 3  # a realization in the 2nd forcing profile (p = 3 // 2 = 1)


def _tiny_config(out_dir: Path) -> MasterEnsembleConfig:
    csv = config.ENSEMBLE_FORCING_MEAN_FRAC_CSV
    if not Path(csv).exists():
        pytest.skip(f"CMIP6 forcing table not available: {csv}")
    return MasterEnsembleConfig(
        master_seed=_SEED,
...
def test_regenerated_realization_matches_staged(tmp_path):
    from src.ensemble_generation import generate_master_ensemble, regenerate_realization

    cfg = _tiny_config(tmp_path / "master")
    manifest = generate_master_ensemble(cfg)

    # Manifest / staged artifacts describe the full master.
    assert manifest.n_realizations == _N_FORCING * _R
    assert manifest.design == "master"
    out = tmp_path / "master"
...
def test_chunked_generation(tmp_path):
    """A chunked master writes K independent chunk ensembles (bounded memory), each global-id mapped."""
    import json
    from src.ensemble_generation import generate_master_ensemble, regenerate_realization
    from src.ensembles import master_chunk_specs

    out = tmp_path / "synthetic_ensembles" / "master_2yr_n16"
    cfg = MasterEnsembleConfig(
        master_seed=_SEED, n_forcing_profiles=8, realizations_per_profile=2,
        realization_years=_L, output_dir=out,
        mean_frac_csv=config.ENSEMBLE_FORCING_MEAN_FRAC_CSV,
        store_daily=True, hazard_block_size=4, chunk_size=8,  # 16 realizations -> 2 chunks of 8
    )
    if not Path(cfg.mean_frac_csv).exists():
        pytest.skip("CMIP6 forcing table not available")
    generate_master_ensemble(cfg)

    # Master dir holds only global artifacts (no daily HDF5); daily lives in sibling chunk dirs.
    ...
    try:
        chunks = master_chunk_specs("master_2yr_n16")
        assert len(chunks) == 2
        ...
        from synhydro import Ensemble
        sh1 = out.parent / "master_2yr_n16__chunk001"
...
def test_hazard_image_and_forcing_profiles_shapes(tmp_path):
    from src.ensemble_generation import generate_master_ensemble
    ...
    assert prof["mean_factor_a"].shape == (n_m, 12)  # one theta row per master realization
