...
All K ensemble draws are staged in ONE job: selection reads only the draw's stored
``hazard_image.npz``, never the pool timeseries, so it is cheap. A draw re-rolls both
sources of construction randomness -- its own pool (regenerated by step 02) and its
anchor plan (``ScenarioDesign.selector_seed(draw)``) -- so draw-to-draw spread measures
the design's full construction variance.
