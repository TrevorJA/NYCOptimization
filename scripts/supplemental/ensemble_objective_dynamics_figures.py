"""ensemble_objective_dynamics_figures.py - Driver for the ensemble suite.

The ensemble analog of ``objective_dynamics_figures.py``. It evaluates the
...
thereafter; the two policies' per-realization simulations are cached under
``supplemental_config.ENSOBJDYN_CACHE_DIR`` so re-runs only re-plot.
...
FIGURES_DIR = scfg.ENSOBJDYN_FIGURES_DIR
CACHE_DIR = scfg.ENSOBJDYN_CACHE_DIR
FORMULATION = "ffmp"
