...
Imported by the sibling ``objective_dynamics_figures.py`` driver, which writes
the figures under ``supplemental_config.OBJDYN_FIGURES_DIR``.
"""
...
def plot_objective_parallel_axes(policies, obj_names, directions, *,
    ...
    **objectives** Borg optimizes, whereas the single historic trace compares
    ...
    labels = [label_for(nm) for nm in obj_names]
    ax.set_xticks(x)
