"""objective_dynamics_figures.py - Driver for the objective-dynamics figures.
...
FIGURES_DIR = scfg.OBJDYN_FIGURES_DIR
CACHE_DIR = scfg.OBJDYN_OUTPUT_ROOT
FORMULATION = "ffmp"
...
def _load_baseline_data(baseline_dv: np.ndarray) -> dict:
    ...
    return _get_or_simulate(baseline_dv, CACHE_DIR / f"baseline_{FORMULATION}.hdf5",
                            "baseline")
...
def main() -> None:
    ...
    contrast_data = _get_or_simulate(
        contrast_dv, CACHE_DIR / "contrast_storage_conservative.hdf5", "contrast")
