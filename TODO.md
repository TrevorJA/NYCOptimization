...
Sequence: the flood-objective adopt/veto (§3) → the epsilon-confirmation rerun
(first item) → step-00 JARs — the objective set, epsilons, and DV bounds are
all baked into the problem definition.
...
- [ ] **[local]** Adopt (or veto) the flood-objective replacement recommended
  2026-08-03 — C4, Σ over days of the max-across-gauges (stage − minor)⁺
  [ft-days/yr] — per the checklist in
  `docs/notes/methods/flood_objective_diagnostics.md` §7: register the §1/§2
  severity metrics, swap `config._DEFAULT_OBJECTIVES` (set stays 8, slugs stay
  `*_obj8`), set the `__sat1` placeholder, then let the §1 epsilon-calibration
  rerun price its ε (provisional 0.01) before JAR generation. Decide BEFORE
  the §1 rerun so the swap rides it at zero extra cost.
