...
- [ ] **[HPC]** Epsilons for the campaign objective set: run the epsilon-calibration experiment (`workflow/supplemental/epsilon_calibration.sh`, one job per campaign design; `docs/notes/methods/epsilon_calibration_experiment.md`) and adopt the combined recommendation into `_ANNUAL_REGISTRY_SPEC` **before JAR generation** — epsilons are baked into the archive. Replaces the 24-policy historic-trace provenance with feasible-policy IQR + bootstrap noise floors on the campaign search measures.
