...
- [ ] **[local]** Reconcile the baseline anchor's simulation path with the search path: the step-05 baseline runs `run_simulation_to_disk` while `evaluate()` runs `run_simulation_inmemory`, and the two disagree by ~5e-4 relative on the same policy and trace (e.g. nyc delivery reliability 0.866483 vs 0.866984) — three orders above the 1e-6 threshold `run_baseline --test-inmemory` itself flags. Distinct from the within-path determinism result above. Decide whether the anchor should be produced on the search path, since `improvement_vs_baseline` differences it against Pareto vectors computed in-memory.
