...
1. **Computational budget.** Both matched designs run at *N* = 100, *L* = 10 yr, which is 1,000 scenario-years per evaluation, at equal NFE (500,000 per search; geometry and SU accounting in `scenario_design_methods.md` §6, single-source constants in the `production` entry of `src/moea_config.py`). Because *N* and *L* are common, per-evaluation simulation cost, scenario-years, and wall-clock are identical, so equal-NFE and equal-scenario-years coincide. There is one budget condition and no confound between ensemble composition and search effort. The common *(N, L)* is required rather than convenient: if *L* differed, the selection rule would be confounded with record length. See `scenario_design_methods.md` §6 for why *N* = 100 is the smallest defensible fill and why long records are not viable at a fixed per-evaluation budget. The statistical adequacy of *N* = 100 as the sample every policy is scored on is measured, not assumed (`ensemble_size_diagnostics.md`): the reliability and flood objectives resolve below the archive's epsilon at *N* ≥ 75 in both designs; the three pooled-percentile operators reach the pre-registered paired-precision criterion (ε/2) only at *N* = 300 for the i.i.d. control and, for hazard filling, at no *N* ≤ 500 on NYC deficit P99 — a shelf-valued operator that the selected extreme members decide. The statistically required common *N* is therefore at least 300 and its upper end is not yet established (the hazard-filling ladder is being extended); whether the campaign adopts it is a budget decision recorded separately. At *N* = 100 the residual sampling noise is quantified in SI Text S5 and absorbed by the draw-level replication below.
