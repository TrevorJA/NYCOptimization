...
**Campaign selection-axis set.** The campaign **selection** axis set is $m = 6$: magnitude, severity, onset rate, recovery rate (dry) and peak discharge, pulse duration (wet). The full retained set cannot meet the per-axis adequacy gate at any affordable pool size (the snap is dimension-limited — improvement exponent ~0.04, far below $P^{-1/8}$; `hazard_selector_diagnostics.md` §5), while this set passes at $P = 10^6$ (min per-axis tail share 0.311; thin margin — re-confirmed per production draw). `drought_duration` and `flood_rise_rate` remain computed in every hazard image and reported post-hoc; they simply do not enter the snap distance. Single source: `config.HAZARD_SELECTION_AXES` (env `NYCOPT_HAZARD_SELECTION_AXES`), consumed by the step-03 selection.
...
**Why $N$ and $L$ are what they are.** The selection comparison requires a *common* $(N, L)$ — if $L$ differed, the selection rule would be confounded with record length. Coverage rests on the LHS anchors' stratified marginals: every selection axis is stratified into $N$ bins regardless of $m$, so the severe tail of each descriptor is over-represented at any dimension, and joint coverage is reported descriptively against a random design of the same $(N, m)$ rather than claimed as uniform filling. $N = 100$ was decided on 2026-07-30 (jointly with $m$ and $P$) on the P = 2,000 laptop N-sweep, which showed per-axis tail enrichment flat-to-declining in $N$ at fixed $P$, and on per-evaluation cost, which scales as $N^{0.95}$ (the budget below is priced at $N = 100$). The ensemble-size diagnostic (`ensemble_size_diagnostics.md`, 2026-08-25) supersedes the selection-level half of that argument: on the production $P = 10^6$ pools the minimum per-axis tail share is flat in $N$ (0.28–0.29 from $N = 50$ to 500; the pool holds ~$10^5$ members above P90 per axis, so selection never depletes the supply) and declines with $N$ only at $P' \le 2\times10^4$. Nothing at the selection level therefore penalizes a larger $N$ at $P = 10^6$; the case for or against a larger $N$ rests on estimator precision against the archive's epsilons (that note's Layer B) and on cost. The same diagnostic found the six-axis adequacy gate a thin miss (0.28 vs 0.30) on the regenerated December-epoch pools, independent of $N$ (`ensemble_size_diagnostics.md` §7.1a; `TODO.md`). Long records are not viable here: at a fixed per-evaluation budget, $L = 50$ forces $N \approx 20$, too few anchors for meaningful stratification. $L = 10$ also exceeds the 1960s DRB drought of record (~4–5 yr) plus onset and recovery, so a design-basis event fits inside a window, and it keeps duration-type hazard axes from being truncation-limited. The per-evaluation ensemble of 100 realizations is smaller than the 1,000-realization convention of the Zeff–Herman–Trindade lineage; the ground for it is the annual-unit pooling of `objective_definitions.md` §2, which yields ~900 metric-bearing unit-years per evaluation, comparable to the sample on which that lineage stabilizes the same tail operators.
...
| Parameter | Status |
|---|---|
| Candidate pool size $P$ | **$P = 10^6$** — the smallest pool passing the adequacy gate for the campaign $m = 6$ set (§3.3). Sharded generation ≈ 600 allocated core-hours per pool (`workflow/supplemental/gen_pool_shards.sh` + `gen_pool_merge.sh`), so K = 3 re-rolls ≈ 2,000 SU — within the §6 allowance. |
...
| Ensemble size $N$ | **OPEN (statistical result 2026-08-26, `ensemble_size_diagnostics.md` §7.3):** the i.i.d. control needs $N = 300$ (worst-pair paired SE $\le \varepsilon/2$ on the deficit-P99 / storage-P01 operators; fitted crossings 210–283); hazard filling meets that criterion at no $N \le 500$ on NYC deficit P99 and its ladder (3 constructions per N) cannot yet distinguish slow decay from a plateau. $N_{\text{common}} \ge 300$, upper end not established; the HF ladder is being extended (more constructions, larger N) before the value is fixed. Adopting any $N \ne 100$ triggers the §7.6 checklist of that note (epsilon re-calibration first). |
| $S$ seeds | Target 2. |
...
- **Estimator precision.** The two designs differ in the variance of their fitness estimates (a frozen i.i.d. ensemble and a frozen coverage-designed ensemble present different sampling structures to the optimizer). Matching $N$ and $L$ removes the compute confound but not this one; it is disclosed, not removed — and now measured (`ensemble_size_diagnostics.md` §7.3): at $N = 100$ the reliability and flood objectives resolve to $\le 0.34\,\varepsilon$ (paired SE) in both designs, the three pooled-percentile operators to $0.7$–$0.8\,\varepsilon$ for the i.i.d. control (reaching $\varepsilon/2$ at $N = 300$), while the hazard-filling construction reproduces NYC deficit P99 across anchor plans only to $2$–$4\,\varepsilon$ at every $N \le 400$ because that operator moves in discrete shelves that the selected extreme members decide. Whether the campaign adopts a larger $N$ is a budget decision outside this note; at $N = 100$ the residual is carried by the draw-level replication.
