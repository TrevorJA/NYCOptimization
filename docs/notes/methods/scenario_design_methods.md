...
**Windows.** Disjoint, non-overlapping $L$-year blocks of whole calendar years, January-aligned: the generator synthesizes calendar-year sequences and every realization is stamped from its true January 1 epoch (`ENSEMBLE_START_DATE`). Each scenario starts from fixed initial storage (`INITIAL_VOLUME_FRAC = 0.80`) — a terminating-simulation design. The metric window of every scenario opens six months after its start: SSI-6 requires six months of accumulation before its first defined value, so both the hazard-selection metrics and the objective metrics ignore that interval, and selection and evaluation therefore score the identical effective window. The cut is applied by date from each window's own calendar, so leap years need no special case. The remainder begins July 1 of year 1; the annual-unit set keeps whole water years only — first unit opening October 1 of year 1, trailing October–December fragment discarded — so an $L$-year realization still yields $L-1$ units, while the daily and weekly metrics start July 1 of year 1. A drought event straddling a window boundary is split; this is bounded by the $L$-vs-design-drought check (§6) and by long $L_{\text{test}}$, where rankings are decided.
