...
**Scope, disclosed:** the hazard space is NYC-aggregate-inflow-only (downstream objective sites include unregulated tributary flow the image never sees; a basin-coverage SI check correlates selected-ensemble hazard coordinates with realized downstream stress) and single-event-per-window (no frequency, timing, or inter-event descriptor enters the selection; event multiplicity is an explicit extension).

**Redundancy handling.** All non-degenerate hazard descriptors are retained as selection axes: degenerate axes (near-zero spread) are dropped, and near-duplicate axes — $\lvert\rho_S\rvert \ge 0.95$ on the pool's hazard image — are pruned to one member, so a single hazard concept cannot enter the snap distance twice under two names. No further reduction is performed: correlated-but-distinct descriptors are deliberately kept, because the design's aim is coverage of the full hydrologic diversity the descriptor set measures, and the selector's coverage guarantee is marginal rather than joint (§4.3). On the laptop diagnostic pool the screen retains all eight candidates — no degenerate axes, and the largest pairwise $\lvert\rho_S\rvert$ is 0.88 (deficit volume–duration), below the cut — with the production-pool screen fixing the final set. The Spearman rank-correlation structure (matrix and $1-\lvert\rho_S\rvert$ cluster tree, Olden & Poff 2003) is reported as a redundancy **diagnostic** alongside the selection, never used to reduce it further; the implicit weighting that correlated axes induce in the snap distance is characterized by a selection-invariance diagnostic (overlap of the selected members as individual axes are added or removed), reported as SI.
...
| Re-evaluation on E_test | full model, all final policies; allowance at the most generous projected E_test grid | ≤ 25,000 |
| **Campaign total** | | **~448,000 (60% of the 750,000-SU allocation)** |
| **Reserve** | | **~302,000** |

Exchange rates for spending the reserve: one additional matched search ≈ 33,400 SU; one additional draw for both matched designs (4 searches) ≈ 134,000 SU; a variable-resolution `ffmp_N` sweep at one design × one draw × two seeds × three N values (6 searches) ≈ 200,000 SU. The reserve covers the sweep *or* roughly two extra draws at full NFE, not both.
...
| Parameter | Status |
|---|---|
| Candidate pool size $P$ | $P \gg N$; dense enough that anchors snap to near neighbours. Report the snap-distance distribution. Production target $10^5$–$10^6$, set by the saturation diagnostic with the adequacy gate: minimum per-axis tail share above the pool P90 $\ge$ ~0.30 at $(m = 8, N = 100)$ — generation SU is not binding (≤ ~9,000 SU total even at $P = 10^6$; see §6 Budget). |
...
| $S$ seeds | Target 2. |
| Hazard-axis count $m$ | Full non-degenerate descriptor set minus near-duplicates ($\lvert\rho_S\rvert \ge 0.95$). On the laptop diagnostic pool ($P = 2{,}000$) the screen retains **all eight candidates** (no degenerate axes; max $\lvert\rho_S\rvert = 0.88$, deficit volume–duration, below the cut even at a 0.90 tightening) and the per-axis coverage guarantee holds on every axis (`hazard_selector_diagnostics.md` §5). The production-pool screen fixes the final set. |
...
- **$m$-vs-$N$ sparsity.** With the full descriptor set retained ($m = 8$ at laptop scale), joint filling at moderate $N$ is not claimed; the intervention rests on stratified marginals and per-axis tail enrichment, and the coverage diagnostics must demonstrate, not assert, that it was administered at strength. The laptop battery (`hazard_selector_diagnostics.md` §5) shows the guarantee holds per axis at $(m = 8, N = 100)$ but is diluted by pool sparsity — snap concentration and per-axis tail enrichment degrade with $m$ at fixed $P$, and raising $N$ at fixed $P$ *lowers* enrichment (finite tail supply). The binding lever is therefore the production pool size $P$, gated by the criterion: minimum per-axis tail share above the pool P90 $\ge$ ~0.30 (≥ ~3× the unbiased 0.10) at $(m = 8, N = 100)$ on the production pool.
