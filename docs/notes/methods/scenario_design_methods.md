...
**Novelty relative to Zaniolo et al. (2023).** DRIPP (ERL 18:054014) is the generation-control relative: training ensembles are *generated to* four discrete SRI-based drought persistence/intensity/frequency types (modified Borgomeo et al. 2015 generator), portfolio policies are trained per type, and a cross-type train/test matrix quantifies the risk/regret of planning for the wrong drought. It establishes that the hazard composition of the training ensemble changes policies, costs, and trigger-indicator structure — motivation for RQ1 — and its intensity-dominance finding supports the §3.3 hazard-axis screen (axis relevance is system-specific). The deltas: hazard properties imposed **at generation** at hand-picked discrete levels vs **selected** from a realized master toward continuous multi-dimensional coverage; four typed ensembles vs a designed composition experiment with budget control; out-of-sample testing from the same four designed families vs a common held-out deeply-uncertain $E_{\text{test}}$. Full note: `docs/notes/literature/notes/Zaniolo et al. (2023).md`.

**Novelty relative to Cohen et al. (2021).** Cohen, Zeff & Herman (EMS 141:105047) is the nearest search-phase antecedent: it shows training-scenario properties drive out-of-sample policy robustness, with high-**baseline-regret** training sets matching all-scenario training at ~3× less compute. It is cited as *motivation* (composition matters; enriching training with high-stress scenarios helps — mechanistic support for hazard-filling's severe-corner over-representation), and differentiated on four deltas: (i) **selection mechanism** — designed space-filling coverage with explicit uniformity/separation criteria vs contrasts of hand-assembled cluster unions; (ii) **property space** — simulation-free hazard metrics computable on sequences alone (reusable across formulations, no per-scenario optimization) vs problem-driven baseline regret (one perfect-foresight optimization per scenario; formulation-specific; unscalable to $\mathcal{M}$); (iii) **pool** — a $10^5$–$10^6$ synthetic master of short sequences vs 97 deterministic GCM traces with no natural-variability realizations; (iv) **evaluation** — a held-out deeply-uncertain $E_{\text{test}}$ (≥2 constructions) vs complementary random halves of the same ensemble. Their §5 future work describes this design almost verbatim. Full note: `docs/notes/literature/notes/Cohen et al. (2021).md`.
...
**Window construction.** Disjoint, non-overlapping $L$-year blocks (avoids the effective-sample-size inflation and autocorrelation of overlapping windows). **$L = 10$ yr**, a single editable config constant. $L$ must exceed the longest within-window drought: the 1960s DRB drought of record is ~4–5 yr, so $L=5$ cannot contain a design-basis event plus onset and recovery, and duration-type hazard axes are truncation-limited at 5 yr; $L=10$ also halves the warm-up overhead share (10% vs 20% of simulated time) and makes SSI-3 event metrics well-defined within windows. Everything downstream of the master (hazard image, failure criteria, subset selections) is $L$-conditional, so changing $L$ means regenerating the master. $L$ is held constant across designs compared on composition; the long-record design (§4.3) varies $L$ and is analyzed as a separate length contrast.
...
Few multi-decadal records at **equal total simulated years** (Quinn et al. 2017). Generate $N'=\lfloor NL/L'\rfloor$ realizations of length $L'=50$ yr (same forcing space as the master, longer realizations) so $N'L'\approx NL$; random draw, fixed across search; $K$ draws × $S$ seeds. Analyzed as a *length contrast* against §4.2, never pooled with the constant-$L$ composition comparison (Brodeur et al. 2020 on overfitting in policy search). Objectives are scored on consecutive annual units with inherited state (Quinn et al. 2018; `objective_definitions.md` §2), so this design shares the common unit denominator $NL$ and needs no operator special-casing; within-record unit dependence is disclosed via the §3.2 ESS/clustered-SE treatment.
...
- **Mechanism hypothesis (supplement only):** higher coverage of the test hazard space by $E_d$ predicts better re-evaluated performance. Operationalized as a supplemental **coverage → re-evaluated-robustness association** (design-level regression of test-set satisficing/hypervolume on coverage; report slope, CI, $R^2$; Holm correction across the ≥2 $E_{\text{test}}$ sets and pooled-vs-leave-one-out reference variants). A search-minus-test overfitting gap is not used: its in-sample term is coverage-weighted for `hazard_filling`, so the gap is never expectation-vs-expectation. The **primary** cross-design comparison is robustness + regret on $E_{\text{test}}$ (`objective_definitions.md` §3); a null mechanism result is reportable.
...
| Scenario length (search) | $L$ | 10 yr | Exceed longest within-window drought (1960s record ~4–5 yr) + onset/recovery; duration axes truncation-limited at 5 yr. |
...
- **Generator stationarity under perturbation.** The Kirsch correlation/NST structure is fit on history and reused under shifted `mean_period`/`std_period` (Kirsch et al. 2013). Historical interannual persistence is retained by design: the deeply-uncertain forcing spans volume, seasonality, and variability but not multi-year drought persistence. Claims are scoped to "under historical persistence"; the hazard-axis reachability screen flags persistence-driven axes as generator-variability-driven; a persistence axis is future work.
