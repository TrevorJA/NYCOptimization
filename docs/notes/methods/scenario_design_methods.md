...
| Parameter | Status |
|---|---|
| Candidate pool size $P$ | **$P = 10^6$** — the pool size at which the campaign $m = 6$ tail share has saturated (0.268 → 0.282 from $10^5$ to $10^6$ at $N = 100$; `ensemble_size_diagnostics.md` §7.1). Sharded generation ≈ 600 allocated core-hours per pool (`workflow/supplemental/gen_pool_shards.sh` + `gen_pool_merge.sh`), so K = 3 re-rolls ≈ 2,000 SU — within the §6 allowance. |
