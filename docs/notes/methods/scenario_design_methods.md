...
Generated: $N_{\theta,\text{test}} = 1{,}000$, $R_{\text{test}} = 25$, $L_{\text{test}} = 50$ yr — 25,000 realizations, 1.25M scenario-years, in 50 staged chunks of 500. Re-evaluated (the campaign): the leading $N_{\theta,\text{reeval}} = 500$ SOWs = the first 25 chunks, 12,500 realizations, 625k scenario-years (`etest_kn_50yr_n25000_first25ch`, a metadata-only prefix subset; LHS rows are randomly ordered, so the prefix is an unbiased, well-spread half of the design, and the incumbent's full cube joins it by SOW label). Both counts are single-source constants in `src/etest.py` and hardcoded nowhere else. The chunked staging/re-evaluation path (`src/chunk_reeval.py`, workflow step 09) exists for exactly this scale. The derivation:

- **Cross-SOW precision governs $N_\theta$** (§5.2). The worst-case Monte Carlo standard error of a satisficing fraction is $0.5/\sqrt{N_\theta}$ = ±2.2 pp at 500. The count is set from the literature by factor-space dimension: 10,000 LHS SOWs in 13-factor spaces (Herman et al. 2014; Trindade et al. 2017) and the 5-factor lake problem (Bartholomew & Kwakkel 2020), 1,000–2,000 at 5–14 factors (Eker & Kwakkel 2018; Hadjimichael et al. 2020; Gold et al. 2023), and, in the one 3-axis space with a measured convergence curve, a 500-scenario database in which satisficing rankings stabilize from 50–300 scenarios and regret-type metrics need 400–500 (Bonham et al. 2024). Our forcing space has 3 axes, so 500 matches that precedent's density and sits at the lower edge of its regret range; the RQ1 discrimination band δ = 2 × paired SE, measured at 0.102 (all-8) / 0.065 (compromise) on the 200-SOW interim cubes, scales to ≈ 0.065 / 0.041 at 500 SOWs.
- **Within-SOW precision is argued in its own terms.** $R_{\text{test}} \times (L_{\text{test}}-1) = 1{,}225$ pooled annual units per SOW resolve each state's objective value at the archive's precision: on the annual-unit library every objective's level standard error falls below its ε from 675 pooled units for an i.i.d. sample (`ensemble_size_diagnostics.md` §7.3), and the per-SOW noise measured directly on $E_{\text{test}}$ from the incumbent (paired near-tie floors of 0.017–0.024 on the reliabilities, 1.2 pp on the deficit operators, 0.04 ft·d/yr on flood, 3.0 pp on storage; `regret_tolerance_diagnostics.md` pass B) is below ε on every axis. $R$ plays no role in cross-SOW precision, which $N_\theta$ sets. Closest structural precedent: Quinn et al. (2020), ~1,050 realization-years per generator parameterization.
- **Long records test sustained operation.** $L_{\text{test}} = 50$ removes the terminating-window reset of search: consecutive droughts are experienced on carried-over storage, and the running-average entitlement banking matures. This asymmetry with the search framing is deliberate, identical across designs, and adds *system-state* continuity, not hydrologic persistence (the §5.1 persistence limitation is unchanged). Hazard-space coordinates of $E_{\text{test}}$ (the overlay figure and the hazard-restricted composition subsets) are computed on disjoint, December-aligned 10-yr sub-windows of each realization, each scored on the pool's exact metric span, keeping them commensurable with the pool's window convention.
- **Trimmed model, priced at the measured rate.** Re-evaluation cost is 33 SU per policy on the 500-SOW prefix (1.33 SU per policy-chunk unit, measured on the step-09 chunked path, × 25 chunks): ~66,000 SU at the 2,000-policy cap of `campaign_design.md` §5 (the post-hoc ε re-filter is coarsened to the cardinality target, identically for every design, if the merged sets exceed it). The one-time full-model presim pass over $E_{\text{test}}$ adds ~70 SU. Trimmed-vs-full objective agreement is demonstrated on the historical trace (all policy × objective pairs agree to $\leq 2.5 \times 10^{-13}$ relative) and reported in SI (S1); because the boundary releases are policy-independent by construction, the historic validation suffices and no per-ensemble re-check is performed.
- **Adequacy is verified, not assumed.** $\theta$-subsample (250 vs 500) and $R$-subsample (5/10/25) ranking-stability curves are scored offline from the persisted matrix (§5.2) at zero simulation cost — the same empirical-convergence move as Trindade et al.'s (2017) 100–5,000 ensemble-size sweep and Quinn et al.'s (2020) 100-vs-1,000 SOW check.
...
**Measured cost and budget.** At 128 evaluator ranks per node on Purdue Anvil, one function evaluation over an $N = 100$, $L = 10$ trimmed-model ensemble takes a median of **173.8 s** (the full model, used only for the presim passes and the single-trace historic baseline, is **1.16×**). The two completed production searches at $N = 100$ (500,000 NFE, 8 × 128) measured **21.3 h and 21,850 SU** each, 1.53× below the 33,400 SU implied by 173.8 s and the 0.729 strong-scaling efficiency, because DV-infeasible decision vectors are rejected before simulation; the measured figure is the cost basis and scales as $(N/100)^{0.951}$ (fitted on $N = 10$–200) and linearly in NFE, with a ≤ 9 % batching penalty. The `historic` reference (~32 s per evaluation on its single 78-yr trace) measured 4,200 SU per 500,000 NFE. Node scaling beyond eight nodes is unmeasured and is carried as a factor of 1.00–1.17 on every 12-node figure.

| Item | Basis | SU |
|---|---|---|
| `fixed_probabilistic` searches | seed 1 (750k) ~102–118k + seed 2 (500k) ~68–79k | ~170–197k |
| `hazard_filling_stationary` searches | same | ~170–197k |
| `historic` reference searches | 750k + 500k at the measured 4,200 per 500k | ~10.5–12.3k |
| Staging, ε re-verification, memory smoke | allowance | ~5k |
| Re-evaluation on E_test | 33 SU per policy measured (500 SOWs), 2,000-policy cap | ~66k |
| Draw-sensitivity re-evaluation (SI) | d1, d2 at $N = 300$ | ~1k |
| **Campaign total** | | **~423–478k** |
| **Reserve against the ~600k balance** | | **~122–177k** |

The full table with the model-basis stress case, the decision points after seed 1, and the S = 2 caveat is `campaign_design.md` §6. A third seed for both matched designs (~135–160k SU of search plus its re-evaluation) does not fit the balance unless the seed-1 runs price well below projection; the RQ3 `ffmp_N` sweep runs only on whatever SU remains at the end of the campaign.

**Pool-size headroom.** Candidate-pool generation is Kirsch–Nowak sampling plus the streamed hazard image — no Pywr-DRB simulation — so its SU cost is bounded by roughly a full-node day (~3,000 SU) per pool even at $P = 10^6$, or ≤ ~9,000 SU across the three staged pools: under 2% of the campaign. $P$ is therefore not SU-constrained; the binding considerations are generation wall-time, pool storage, and selector behavior, which is why $P$ is set by the saturation diagnostic rather than by budget.
...
Scenario discovery, where it is run, operates in the **DU factor space** of $E_{\text{test}}$ after re-evaluation (`objective_definitions.md` §4) as an **optional supporting analysis**, not the primary comparison and not a falsification result; no scenario discovery is performed in hazard space.

### Open parameters

| Parameter | Status |
|---|---|
| Candidate pool size $P$ | **$P = 10^6$** — the pool size at which the campaign $m = 6$ tail share has saturated (0.268 → 0.282 from $10^5$ to $10^6$ at $N = 100$; `ensemble_size_diagnostics.md` §7.1). Sharded generation ≈ 600 allocated core-hours per pool (`workflow/supplemental/gen_pool_shards.sh` + `gen_pool_merge.sh`), so three re-rolls ≈ 2,000 SU — within the §6 allowance. |
