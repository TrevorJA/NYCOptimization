...
Rows are ordered by design family, from single reference traces through Monte Carlo sampling, stratified and resampled designs, multi-scenario deep-uncertainty designs, and adaptive selection.
...
5. Löhndorf (2016) is the only systematic head-to-head comparison of scenario-design methods judged by optimization error, and Bartholomew & Kwakkel (2020) the only head-to-head of search-phase scenario treatments with common re-evaluation. Neither runs a controlled contrast of hazard-space coverage against i.i.d. Monte Carlo sampling at matched N, L, and NFE on a real system under a held-out deeply uncertain re-evaluation, which is the experiment this study contributes.
