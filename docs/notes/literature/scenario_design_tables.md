...
5. Löhndorf (2016) is the only systematic head-to-head comparison of scenario-design methods judged by optimization error, and Bartholomew & Kwakkel (2020) the only head-to-head of search-phase scenario treatments with common re-evaluation. Neither compares design families spanning probabilistic, input-space, and hazard-space construction at matched budget on a real system, which is the experiment this study contributes.
