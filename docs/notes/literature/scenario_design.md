...
**Construction.** Each design is built to its own recipe; there is no shared pool from which designs are drawn. `hazard_filling` owns a **candidate pool** of i.i.d. realizations from which its N members are selected; the Monte Carlo comparison is generated directly. The pool is disjoint from the held-out test ensemble E_test, which is where the large ensemble now lives: a large ensemble is used **only for post-optimization re-evaluation**, never as a source for the search ensembles.
...
1. `historic` — the observed record (N = 1, one 78-yr trace scored as 77 annual units); prevailing-practice reference, deliberately unmatched (Giuliani et al. 2016; Herman et al. 2020).
2. `monte_carlo` — N × L drawn i.i.d. from the stationary generator, frozen across the search; the random-sampling control (Quinn et al. 2017; Zatarain Salazar et al. 2017).
...
Designs 2 and 3 both use N = 300 and L = 10 yr (3,000 scenario-years per evaluation, equal NFE); `historic` is deliberately unmatched. A common (N, L) is required or the selection rule is confounded with record length; N is bounded below by the marginal-coverage requirement (LHS stratifies every retained axis into N bins), with the final N fixed by the ensemble-size diagnostics (`../methods/ensemble_size_diagnostics.md`, `../methods/campaign_design.md`). Per-design precedents and the declared deviations from them are tabulated in [scenario_design_tables.md](scenario_design_tables.md), Table 3.

**The controlled contrast.** 2 → 3: same generator, same population law, same N, same L; only the *selection rule* changes. Because a uniform random size-N subset of an i.i.d. pool has exactly the joint law of N fresh i.i.d. draws, `monte_carlo` is the *exact* statistical control for `hazard_filling`, and any difference in re-evaluated robustness is attributable to the selection rule alone. This is the Eker & Kwakkel (2018) null benchmark ("diversity-based selection did not beat random") upgraded to hazard space and a real system, with seed-replicated searches (the seed as the unit of analysis, one search draw per design) and draw-dependence measured by re-evaluating each design's final set on its other draws. `historic` anchors prevailing practice as a reference point outside the matched contrast.

## Terminology

Three spaces, per [terminology.md](../terminology.md): the **input space** (generator parameters, climate/forcing multipliers — the "states of the world"); the **hazard space** (hydrologic hazard metrics computed on each realized sequence — the six selection axes of drought magnitude, severity, onset rate, recovery rate, flood peak discharge and pulse duration, with drought duration and flood rise rate computed only); the **outcome space** (simulation outputs — objective/performance values). The scenario-neutral literature's "exposure space" sits between input and hazard space. The novelty is designing coverage of the *hazard space* directly rather than the input space, since distinct input-space samples yield realizations with overlapping, redundant hazard characteristics.
...
**Related conceptual notes:** [scenario_design_taxonomy.md](scenario_design_taxonomy.md) · [scenario_design_tables.md](scenario_design_tables.md) · [../methods/experimental_design.md](../methods/experimental_design.md)
